...
fn write_print_settings(writer: &mut InternalWriter, worksheet: &Worksheet, r_id: i32) -> i32 {
    ...
    if worksheet.page_setup().has_param() {
        ...
        // ETB PATCH (2026-07-03): 旧実装は has_param() が真なら常に r_id を1消費していたが、
        // worksheet_rels.rs 側は object_data().is_some() のときだけ printerSettings 関係を
        // 割り当てる。ページ設定はあるが埋め込み印刷バイナリを持たないシート
        // （用紙サイズ・向き指定のみ等、ごく一般的なケース）でこの2つが食い違い、
        // 後続の legacyDrawing/comments 等の r:id がすべて1つズレて、Excelが
        // 「一部の内容に問題がある」として図形やコメントを削除してしまうバグの原因だった。
        // worksheet_rels.rs の条件（object_data().is_some()）に完全一致させることで解消する。
